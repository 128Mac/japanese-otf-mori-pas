...
sub is_dvicode($){
	...
	if ($key eq 'hira-de')  { return ($code == 0x2447);}
	...
	if ($key eq 'em-dash')   { return ($code == 0x213D);}
	...
	if ($key eq 'kanakakko') { return 
		(0x2156 <= $code && $code <= 0x2159);}
	...
	if ($key eq 'hira-de')  { return ($code == 0x3067);} # :
	...
	if ($key eq 'em-dash')   { return ($code == 0x2014 || $code == 0x2015);}
	if ($key eq 'quote') { return 
...
sub is_ucs_kigo{
	...
	return 1 if ($dvicode>=0x2014 && $dvicode<=0x2015);
